...
registerBlockType('nhsblocks/contentslistpage', {
	...
	edit: withSelect((select) => {
		...
		const addToArray = (block) => {
			if (block.name === 'core/heading' && block.attributes.level === 2) {
				const anchor = 'nhs-block-anchor-' + index++;
		...
		const is_same =
			h2titles.length == dynamicArray.length &&
			h2titles.every(function (element, index) {
				return (
					dynamicArray[index].text === element.text &&
					dynamicArray[index].url === element.url
				);
			});
				...
				>
					...
					<ol className="nhsuk-contents-list__list">
						{h2titles ? (
							...
							<p>
								Press the button below to make a table of
